 ...
 */
public interface BufferedNativeSink extends NativeSink {
  /** Returns this sink's internal buffer. */
  NativeBuffer buffer();

  BufferedNativeSink write(NativeByteString byteString) throws IOException;

  /**
   * Like {@link OutputStream#write(byte[])}, this writes a complete byte array to
   * this sink.
   */
  BufferedNativeSink write(byte[] source) throws IOException;

  /**
   * Like {@link OutputStream#write(byte[], int, int)}, this writes {@code byteCount}
   * bytes of {@code source}, starting at {@code offset}.
   */
  BufferedNativeSink write(byte[] source, int offset, int byteCount) throws IOException;
   ...
   */
  long writeAll(NativeSource source) throws IOException;

  /** Removes {@code byteCount} bytes from {@code source} and appends them to this sink. */
  BufferedNativeSink write(NativeSource source, long byteCount) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeUtf8(String string) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeUtf8(String string, int beginIndex, int endIndex) throws IOException;

  /** Encodes {@code codePoint} in UTF-8 and writes it to this sink. */
  BufferedNativeSink writeUtf8CodePoint(int codePoint) throws IOException;

  /** Encodes {@code string} in {@code charset} and writes it to this sink. */
  BufferedNativeSink writeString(String string, Charset charset) throws IOException;

  /**
   * Encodes the characters at {@code beginIndex} up to {@code endIndex} from {@code string} in
   * {@code charset} and writes it to this sink.
   */
  BufferedNativeSink writeString(String string, int beginIndex, int endIndex, Charset charset)
      throws IOException;

  /** Writes a byte to this sink. */
  BufferedNativeSink writeByte(int b) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeShort(int s) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeShortLe(int s) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeInt(int i) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeIntLe(int i) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeLong(long v) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeLongLe(long v) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeDecimalLong(long v) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink writeHexadecimalUnsignedLong(long v) throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink emit() throws IOException;
   ...
   * }</pre>
   */
  BufferedNativeSink emitCompleteSegments() throws IOException;
